...
func Test_Execute_Success(t *testing.T) {
	repository := newFakeRepo([]model.ProductReview{
		newReview(1, "PROD-001", "CUST-001", "ORD-001", 5, time.Date(2026, 5, 2, 9, 0, 0, 0, time.Local)),
		newReview(2, "PROD-001", "CUST-002", "ORD-002", 4, time.Date(2026, 5, 3, 9, 0, 0, 0, time.Local)),
	})
	usecase := NewGetList(repository, service.NewProductReviewService())

	result := usecase.Execute(context.Background(), GetListRequest{ProductID: "PROD-001"})
...
func (r *fakeRepo) FindByProductIDOrderByCreatedAtDesc(productID string) []model.ProductReview {
	reviews := filterReviews(r.reviews, func(review model.ProductReview) bool { return review.ProductID == productID })
